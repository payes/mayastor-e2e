...
func (cp CPv1oa) GetMSV(uuid string) (*common.MayastorVolume, error) {
	...
	if volSpec.GetNumReplicas() < 1 {
		return nil, fmt.Errorf("GetMsv msv.Spec.Num_replicas=\"%v\"", volSpec.GetNumReplicas())
	}
...
func (cp CPv1oa) GetMsvNodes(uuid string) (string, []string) {
...
func (cp CPv1oa) DeleteMsv(uuid string) error {
	return cp.oa.deleteVolume(uuid)
}

func (cp CPv1oa) ListMsvs() ([]common.MayastorVolume, error) {
	vols, err := cp.oa.getVolumes()
	if err != nil {
...
func (cp CPv1oa) SetMsvReplicaCount(uuid string, replicaCount int) error {
	return cp.oa.putReplicaCount(uuid, replicaCount)
}

func (cp CPv1oa) GetMsvState(uuid string) (string, error) {
	vol, err, _ := cp.oa.getVolume(uuid)
	var volState string
...
func (cp CPv1oa) GetMsvReplicas(uuid string) ([]common.Replica, error) {
	var replicas []common.Replica

	vol, err, _ := cp.oa.getVolume(uuid)
	if err == nil {
		for rUuid, rt := range vol.State.ReplicaTopology {
			r, _ := cp.oa.getReplica(rUuid)
...
func (cp CPv1oa) GetMsvNexusChildren(uuid string) ([]common.NexusChild, error) {
...
func (cp CPv1oa) GetMsvNexusState(uuid string) (string, error) {
...
func (cp CPv1oa) IsMsvPublished(uuid string) bool {
	vol, err, _ := cp.oa.getVolume(uuid)
	if err == nil {
...
func (cp CPv1oa) IsMsvDeleted(uuid string) bool {
	_, err, responseStatusCode := cp.oa.getVolume(uuid)
	return err != nil && responseStatusCode == 404
}

func (cp CPv1oa) CheckForMsvs() (bool, error) {
	vols, err := cp.oa.getVolumes()
	return err == nil && vols != nil && len(vols) != 0, err
}

func (cp CPv1oa) CheckAllMsvsAreHealthy() error {
	allHealthy := true
	vols, err := cp.oa.getVolumes()
