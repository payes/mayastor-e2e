...
func DeployFio(clientset kubernetes.Clientset, fioPodName string, pvcName string, volumeType VolumeType, volSize int, fioLoops int) error {
	...
	pod, err := CreatePod(clientset, podObj, NSDefault)
	if err != nil {
	...
	for ix := 0; ; ix++ {
		if IsPodRunning(clientset, fioPodName, NSDefault) {
			break
		}
